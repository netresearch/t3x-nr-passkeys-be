...
final class EnforcementServiceTest extends TestCase
{
    ...
    {
        $userRow = ['uid' => 5, 'usergroup' => '1,2,3', 'passkey_grace_period_start' => 0];
    ...
    {
        $userRow = ['uid' => 10, 'usergroup' => '1', 'passkey_grace_period_start' => 0];
        $this->setUpGroupQuery([['uid' => 1, 'passkey_enforcement' => 'encourage', 'passkey_grace_period_days' => 30]]);
        $this->credentialRepository
            ->expects(self::once())
            ->method('countByBeUser')
            ->with(10)
    ...
    {
        $userRow = ['uid' => 7, 'usergroup' => '1,2,3', 'passkey_grace_period_start' => 0];
        $this->setUpGroupQuery(
            [
    ...
    {
        $userRow = ['uid' => 8, 'usergroup' => '1,2', 'passkey_grace_period_start' => 0];
        $this->setUpGroupQuery(
            [
    ...
    {
        $userRow = ['uid' => 9, 'usergroup' => '1', 'passkey_grace_period_start' => 1700000000];
        $this->setUpGroupQuery([['uid' => 1, 'passkey_enforcement' => 'required', 'passkey_grace_period_days' => 14]]);
        $this->credentialRepository
            ->expects(self::once())
            ->method('countByBeUser')
            ->with(9)
    ...
    public function startGracePeriodUpdatesBeUsersTable(): void
    {
        ...
        $connection
            ...
            ->with(
                'be_users',
                self::callback(
                    static fn(array $data): bool => isset($data['passkey_grace_period_start']) && \is_int($data['passkey_grace_period_start']) && $data['passkey_grace_period_start'] > 0,
                ),
                ['uid' => 42],
            );
        $this->connectionPool
    ...
    {
        $userRow = ['uid' => 11, 'usergroup' => '1,2', 'passkey_grace_period_start' => 0];
        $this->setUpGroupQuery(
            [
    ...
    {
        $userRow = ['uid' => 12, 'usergroup' => '1'];
        $this->setUpGroupQuery([['uid' => 1, 'passkey_enforcement' => 'encourage', 'passkey_grace_period_days' => 7]]);
        $this->credentialRepository
            ->expects(self::once())
            ->method('countByBeUser')
            ->with(12)
