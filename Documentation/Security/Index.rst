...
1. A **32-byte random challenge** generated by
   ``random_bytes(32)``
2. An **expiration timestamp** (configurable TTL, default
   120 seconds)
3. A **single-use nonce** (32 hex characters from
   ``random_bytes(16)``)
...
2. If found, it is immediately invalidated (removed from
   cache).
3. If not found (already used or expired), the verification
   fails.
...
the configured threshold (:confval:`rateLimitMaxAttempts`,
default: 10) is exceeded within the time window
(:confval:`rateLimitWindowSeconds`, default: 300 seconds),
...
threshold (:confval:`lockoutThreshold`, default: 5), the
account is locked for the configured duration
(:confval:`lockoutDurationSeconds`, default: 900 seconds /
15 minutes).

A separate per-username threshold
(:confval:`lockoutUserThreshold`, default: 15) counts
...
When :confval:`disablePasswordLogin` is enabled, users
...
..  seealso::
    ...
    :ref:`Production deployment requirements
